...
CREATE TABLE IF NOT EXISTS `Position` (
	...
	`PositionName` ENUM('Developer', 'Dev2', 'PM', 'Leader', 'Scrum_Master' , 'Test'),
...
SELECT DepartmentID, DepartmentName FROM Department WHERE DepartmentName LIKE '%Sale%';
...
SELECT FullName, character_length(Fullname) AS `l` FROM `Account` WHERE DepartmentID = 3 ORDER BY `l` DESC;

SELECT AccountID, FullName, DepartmentID, PositionID From `Account` Order by DepartmentID Desc, positionID asc;  

/* loại bỏ các giá trị trùng lặp chỉ lấy 1 bản ghi duy nhất */ 
SELECT * FROM `Account` ORDER BY DepartmentID, positionID;

SELECT DISTINCT DepartmentID From `Account`;

SELECT * FROM Department WHERE DepartmentID IN ( SELECT)


SELECT * FROM `Account` ORDER BY DepartmentId , PositonID;
/* Đếm trong bảng account mỗi phòng ban có bao nhiêu người*/
SELECT DepartmentID, count(*) FROM `Account` GROUP BY(DepartmentID);

SELECT DepartmentID, count(*) AS `Total` FROM `Account` GROUP BY(DepartmentID);

SELECT Username, DepartmentID, count(1) AS `Total` FROM `Account` GROUP BY(DepartmentID) ;

SELECT DepartmentID, positionID, count(1) AS `Total` FROM `Account` GROUP BY DepartmentID , positionID  ;

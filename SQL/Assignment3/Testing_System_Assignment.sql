...
SELECT CategoryID, count(*) AS `sl` FROM question GROUP BY (CategoryID);

SELECT CategoryID,CategoryName, count(*) AS `sl` 
FROM question JOIN categoryquestion USING(categoryID) GROUP BY (CategoryID);
...
SELECT DP.DepartmentName, DP.PositionName, A.AccountID, A.UserName FROM
...
-- 
SELECT * FROM Department WHERE DepartmentID IN(SELECT DISTINCT DepartmentID FROM `Account`);
-- đếm trong bảng accont mỗi phòng ban có bao nhiêu người 
SELECT DepartmentID, COUNT(*) AS `Total` FROM `account` GROUP BY(DepartmentID);

-- lỗi Group by User name => muốn group by cột nào thì select cột đó 
SELECT UserName, DepartmentID, COUNT(1) AS `Total` FROM `account` GROUP BY(DepartmentID);

-- group by 2 cột 
SELECT * FROM `account`;

SELECT DepartmentID, PositionID, count(*) AS `total` FROM `Account` GROUP BY DepartmentID, PositionID;
